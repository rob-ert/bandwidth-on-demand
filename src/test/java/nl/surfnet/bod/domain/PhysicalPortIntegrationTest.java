...
public class PhysicalPortIntegrationTest {
	...
	public void testCountAllPhysicalPorts() {
		Assert.assertNotNull(
		    "Data on demand for 'PhysicalPort' failed to initialize correctly",
		    dod.getRandomPhysicalPort());
		long count = physicalPortService.countAllPhysicalPorts();
		Assert
		    .assertTrue(
		        "Counter for 'PhysicalPort' incorrectly reported there were no entries",
		        count > 0);
	}
	...
	public void testFindPhysicalPort() {
		...
		Assert
		    .assertNotNull(
		        "Data on demand for 'PhysicalPort' failed to provide an identifier",
		        id);
		obj = physicalPortService.findPhysicalPort(id);
		Assert.assertNotNull(
		    "Find method for 'PhysicalPort' illegally returned null for id '" + id
		        + "'", obj);
		Assert.assertEquals(
		    "Find method for 'PhysicalPort' returned the incorrect identifier", id,
		    obj.getId());
	}
	...
	public void testFindPhysicalPortEntries() {
		...
		List<PhysicalPort> result = physicalPortService.findPhysicalPortEntries(
		    firstResult, maxResults);
		Assert.assertNotNull(
		    "Find entries method for 'PhysicalPort' illegally returned null",
		    result);
		Assert
		    .assertEquals(
		        "Find entries method for 'PhysicalPort' returned an incorrect number of entries",
		        count, result.size());
	}
	...
	public void testFlush() {
		...
		Assert.assertTrue(
		    "Version for 'PhysicalPort' failed to increment on flush directive",
	...
	public void testUpdatePhysicalPortUpdate() {
		...
		Assert
		    .assertTrue(
		        "Version for 'PhysicalPort' failed to increment on merge and flush directive",
		        (currentVersion != null && obj.getVersion() > currentVersion)
		            || !modified);
	}
	...
	public void testSavePhysicalPort() {
		...
		Assert
		    .assertNotNull(
		        "Data on demand for 'PhysicalPort' failed to provide a new transient entity",
		        obj);
		Assert.assertNull("Expected 'PhysicalPort' identifier to be null",
		    obj.getId());
		physicalPortService.savePhysicalPort(obj);
		physicalPortRepo.flush();
		Assert.assertNotNull(
		    "Expected 'PhysicalPort' identifier to no longer be null", obj.getId());
	}
